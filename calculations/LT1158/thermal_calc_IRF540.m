...
% Calculation for IRF540z (High side)
% Based on application note AN 5483

% Data
T_pcbmax = 100; % FR-4 pcb temperature max [C]
T_ambmax = 50; % max ambient temperature [C]
Rth_j_c = 1.15; % junction-case [C/W]

% Thermal resistance
Rth_c_s = 0.5; % Junction to sink [C/W]
Rth_s_a = 10; % Sink to ambient Aavid 7021B-MT
...
f_pwm = 20 * 10^3; % PWM switchin frequency [Hz]
Rdson = 26.5 * 10^(-3); % Rdson mOhm
...
% Switching losses [W]
Vgate = 47; % Gate drive voltage

% Switching loses according to AN 5483
P_switch_loss = V_bat * I_aver * (t_fall + t_rise) * 0.5 * f_pwm;

% Switching losses according to Analog Devices
P_switch_loss_ad = (Crss * Vgate^2 * f_pwm * I_load)/Igate;

% Conduction losses [W]
P_cond_loss = I_aver^2 * Rdson;

% Total losses
P_total = P_switch_loss_ad + P_cond_loss;
% Junction temperature calculate
T_j = P_total * Rth_j_a + T_ambmax;
...
% Nominator
Rdson_nom = (150 * ( T_pcb - T_ambmax - P_switch_loss_ad * Rth_j_c));
% Final value
Rdson_max = Rdson_nom / Rdson_denom;

% Conclusion:
% Calculated value of Rdson_max = 30.7 mOhm 
% IRF540z Rdson = 26.5 mOhm
% Assumed maximum PCB temperature : 100 C degree